...
- 작업 시작 전에 반드시 계획 DAG를 구상해 사용자에게 승인을 받는다. 예외는 다음과 같다.
  - 사용자가 "직접 수행하라"고 명시적으로 요청했거나, 직접 DAG를 제공한 경우
  - 본인이 서브에이전트로서 메인에이전트에게 업무를 위임받은 경우
	- read-only 업무인 경우
- 이후 각 DAG의 단위작업마다 서브에이전트에게 위임해 실행하고 감독업무만 하라. 이때 가능한 경우 병렬처리 가능한 계획을 세워라. DAG는 ASCII 아트로 시각화하지 않고 핵심을 설명한다.
	- 나쁜 예시: A 객체를 리팩토링해.
  - 좋은 예시: (기존 사용처를 찾는다), (새 객체를 정의한다) -|모두끝나면|-> (교체한다) -|모두끝나면|-> (테스트를 수행한다)
